...
10   /*
10 !           */
11   /*  PURPOSEF:        THE PURPOSE OF THIS PROGRAM IS TO TRACE THE JAN&FEB IN 
11 !                      THE FISCAL YEAR comfisc            */
...
31
32   LIBNAME FL_DEC 'H:\navy_fmo\Audit Readiness\SOURCE DATA\STARS-FL\FY\DEC\SAS
32 ! Datasets';
NOTE: Libref FL_DEC was successfully assigned as follows:
      Engine:        V9
      Physical Name: H:\navy_fmo\Audit Readiness\SOURCE DATA\STARS-FL\FY\DEC\SAS
      Datasets
33   LIBNAME FL_JAN 'H:\navy_fmo\Audit Readiness\SOURCE DATA\STARS-FL\FY\JAN\SAS
33 ! Datasets';
NOTE: Libref FL_JAN was successfully assigned as follows:
      Engine:        V9
      Physical Name: H:\navy_fmo\Audit Readiness\SOURCE DATA\STARS-FL\FY\JAN\SAS
      Datasets
34   LIBNAME FL_FEB 'H:\navy_fmo\Audit Readiness\SOURCE DATA\STARS-FL\FY\FEB\SAS
34 ! Datasets';
NOTE: Libref FL_FEB was successfully assigned as follows:
      Engine:        V9
      Physical Name: H:\navy_fmo\Audit Readiness\SOURCE DATA\STARS-FL\FY\FEB\SAS
      Datasets
35   LIBNAME FL_MAR 'H:\navy_fmo\Audit Readiness\SOURCE DATA\STARS-FL\FY\MAR\SAS
35 ! Datasets';
...
73   %MEND;
74
75   *%CONTROL_TOTALS(FL_JAN.FL_FY_JAN,AMOUNT_USSGL_FINAL);
76   /*UTILIZE THE STARS-FL JAN&FEB FY POPULATION AND LIMIT TO TRANSACTIONS TO
76 ! CERTAIN GL4'S BASED ON CFMS EXECUTION CODES*/
77
78   DATA FL_RECON;
79       SET FL_JAN.FL_FY_JAN FL_FEB.FL_FY_FEB;
...
NOTE: There were 7569388 observations read from the data set FL_JAN.FL_FY_JAN.
      WHERE (APN_BEG_FIS_YR='13') and (SUBSTR(GL4, 1, 1)='4') and (GA='17');
NOTE: There were 7844462 observations read from the data set FL_FEB.FL_FY_FEB.
      WHERE (APN_BEG_FIS_YR='13') and (SUBSTR(GL4, 1, 1)='4') and (GA='17');
...
87       SET FL_DEC.FL_FY_DEC;
88       where APN_BEG_FIS_YR IN ('13') and
89       SUBSTR(GL4,1,1)='4'
90       and GA='17';
91   RUN;

NOTE: There were 6826264 observations read from the data set FL_DEC.FL_FY_DEC.
      WHERE (APN_BEG_FIS_YR='13') and (SUBSTR(GL4, 1, 1)='4') and (GA='17');
...
290        SET FL_MAR.fl_FY_mar;
291        where APN_BEG_FIS_YR IN ('13') and
292        SUBSTR(GL4,1,1)='4'
293        and GA='17';
294  RUN;

NOTE: There were 7940668 observations read from the data set FL_MAR.FL_FY_MAR.
      WHERE (APN_BEG_FIS_YR='13') and (SUBSTR(GL4, 1, 1)='4') and (GA='17');
...
354
355  DATA FL_NON_BUD;
356      SET FL_JAN.FL_FY_JAN FL_FEB.FL_FY_FEB FL_DEC.FL_FY_DEC
356! FL_MAR.fl_FY_mar;
357      where APN_BEG_FIS_YR IN ('13') and
358      SUBSTR(GL4,1,1) NE '4'
359      and GA='17';
360  RUN;

NOTE: There were 12393788 observations read from the data set FL_JAN.FL_FY_JAN.
      WHERE (APN_BEG_FIS_YR='13') and (SUBSTR(GL4, 1, 1) not = '4') and (GA='17');
NOTE: There were 12921672 observations read from the data set FL_FEB.FL_FY_FEB.
      WHERE (APN_BEG_FIS_YR='13') and (SUBSTR(GL4, 1, 1) not = '4') and (GA='17');
NOTE: There were 11089951 observations read from the data set FL_DEC.FL_FY_DEC.
      WHERE (APN_BEG_FIS_YR='13') and (SUBSTR(GL4, 1, 1) not = '4') and (GA='17');
NOTE: There were 13220263 observations read from the data set FL_MAR.FL_FY_MAR.
      WHERE (APN_BEG_FIS_YR='13') and (SUBSTR(GL4, 1, 1) not = '4') and (GA='17');
